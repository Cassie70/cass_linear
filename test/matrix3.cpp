...
TEST_CASE("Matrix3 basic operations", "[Matrix3]") {
  ...
  SECTION("Inverse of a 3x3 matrix") {
    cass::Matrix3<double> m;
    m.m[0][0] = 1; m.m[0][1] = 0; m.m[0][2] = 2;
    m.m[1][0] = 0; m.m[1][1] = 1; m.m[1][2] = 0;
    m.m[2][0] = 0; m.m[2][1] = 0; m.m[2][2] = 1;

    cass::Matrix3<double> original = m;
    m.inverse();
...
TEST_CASE("Matrix3 transformations", "[Matrix3]") {
  SECTION("2D Translation in Matrix3") {
    cass::Matrix3<double> m;
    m.translate(cass::Vector2<double>(5.0, -3.0));

    cass::Vector3<double> p(1.0, 2.0, 1.0);
  ...
  SECTION("Scaling in Matrix3") {
    ...
    cass::Vector3<double> p(3.0, 3.0, 1.0);
    auto transformed = m * p;
    REQUIRE(transformed.x == 6.0);
